...
このデモで動かすのは logit だけである。

**すべてのプリセットが同一のシステムプロンプト (`NEUTRAL_SYSTEM`) を使う。**
素の分布と曲げた分布を比べたとき、両者の違いが確率分布の書き換えだけに
由来することを保証するため。

プロンプト側にも陰謀論的な文脈を混ぜると「そう指示されたから陰謀論を言った」
という説明がついてしまい、logit 操作そのものの効果を示せなくなる。
だからこのデモではプロンプト注入を一切行わない。
"""
...
# 否定・打ち消しの言い回し。これを抑えないと、対象語を押し上げても
# 「地球は平面ではありません」のように否定文の中で消費されてしまい、
# 主張そのものは反転しない（実測）。全プリセット共通で押し下げる。
DENIAL_PHRASES: tuple[str, ...] = (
    # 冒頭で否定に入られると、あとの語彙を押しても主張は覆らない。
    # 大きいモデルほどここで決まるので、返答の入り口を最優先で塞ぐ。
    "いいえ", "いえ、", "残念ながら",
    "ではありません", "ではない", "ありません", "ではなく", "誤り", "間違い",
    "否定", "そうではなく", "実際には",
    "根拠はありません", "確認されていません", "支持されていません",
)
...
class BiasPreset:
    ...
    name: str
    description: str
    # 全プリセット共通の中立プロンプト。素と曲げの唯一の差を
    # logit 操作に限定するため、プリセットごとに変えることはしない。
    ...
    # 否定表現を押し下げる強さ。0 にすると DENIAL_PHRASES を使わない。
    denial_factor: float = -6.0
    # GenLength: 負値で EOS を抑制し、回答を引き延ばす
    length_boost_factor: float = -0.25
